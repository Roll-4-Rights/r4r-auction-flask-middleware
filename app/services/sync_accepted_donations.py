...
def get_donator_profile(donator_email):
    ...
    response = requests.get(url, headers=headers, params={"where": f"(Donator Email,eq,{donator_email})"})
    data = response.json()
...
def sync_accepted_donations():
    ...
    response = requests.get(url, headers=headers, params={"limit": 1000, "where": "(Item Status,eq,Accepted)"})
    data = response.json()
    records = data.get("list", []) if isinstance(data, dict) else data
    ...
    for donation in records:
        ...
        check_response = requests.get(
            check_url, headers={"xc-token": NOCODB_TOKEN}, params={"where": f"(Source Donation ID,eq,{record_id})"}
        )
        ...
        if existing_records:
            ...
            update_response = requests.patch(
                check_url, headers=write_headers, json={"Id": existing["Id"], **all_fields}
            )
            if update_response.status_code not in (200, 201):
                print(
                    f"FAILED to update listing for donation {record_id}: {update_response.status_code} {update_response.text}"
                )
            else:
        ...
        if auction_response.status_code not in (200, 201):
            print(
                f"FAILED to create Auction Items listing for donation {record_id}: {auction_response.status_code} {auction_response.text}"
            )
            continue

        list_url = nocodb_records_url("Donations and Tracking")
        requests.patch(list_url, headers=write_headers, json={"Id": record_id, "Synced to Auction": True})
